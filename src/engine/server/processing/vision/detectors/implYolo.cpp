...
Detections SimpleVision::detect(const cv::Mat& frame) {
    ...
    #ifdef DEBUG
    std::cout << "[DEBUG] Frame type: " << frame.type() << " | Channels: " << frame.channels() << std::endl;

    // Print the blob shape
    std::cout << "[DEBUG] Blob shape (dims=" << blob.dims << "): ";
    ...
    std::cout << "[DEBUG] Unconnected Out Layers Names:" << std::endl;
    ...
    #ifdef DEBUG
    std::cout << "[DEBUG] Rows: " << rows << " Dimensions: " << dimensions << std::endl;
    #endif
